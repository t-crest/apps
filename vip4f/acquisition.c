...
volatile _UNCACHED long counter;

void display(struct vip4f_t *vip4f, int current_time) 
{
  long local_counter = counter;//vip4f->counter;
  ...
  /* // TODO: this is part of the original trace but useless ...   */
...
void agARGA(void *arg)  {
  ...
  while (1) {
    ...
    // If needed, display the previous iteration
    if (vip4f->counter != -1) {
    ...
    start_time = *timer_ptr;
    vip4f->DataBufferI[counter % D_TRS_NB_ECH_FILTRE][D_ACQ_VOIE_I1] = sinus_data100A[cmpt][D_ACQ_VOIE_I1];
    vip4f->DataBufferI[counter % D_TRS_NB_ECH_FILTRE][D_ACQ_VOIE_I2] = sinus_data100A[cmpt][D_ACQ_VOIE_I2];
    vip4f->DataBufferI[counter % D_TRS_NB_ECH_FILTRE][D_ACQ_VOIE_I3] = sinus_data100A[cmpt][D_ACQ_VOIE_I3];
