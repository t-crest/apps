...
//    	csslStructure->fd=open(fname,O_RDWR|O_NOCTTY|O_NONBLOCK|O_NDELAY);
//	fcntl(csslStructure->fd,10,CSSL_SIGNAL);
//	fcntl(csslStructure->fd,8,getpid());
//	fcntl(csslStructure->fd,4,O_ASYNC|O_NONBLOCK);
//    } else 
    {
...
//    }
    ...
    tcgetattr(csslStructure.fd,&(csslStructure.oldtio));
    
    /* now we set new values */
    cssl_setup(&csslStructure,baud,parity,bits,stop);

    /* and id */
    csslStructure.id=id;

    /* then set the callback */
    csslStructure.callback=callback;
    
//    /* we add the serial to our list */
//    csslStructure.next=head;
   // head=csslStructure;
	
    cssl_error=CSSL_OK;

    return &csslStructure;
}
